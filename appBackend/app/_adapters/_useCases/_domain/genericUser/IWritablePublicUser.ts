import {
  checkNonStrictWritablePublicGenericObject,
...
export const checkNonStrictWritablePublicUser = (
  ...
  if (request.mainLanguage) {
    request.mainLanguage = Validator.validateLength(
      { value: request.mainLanguage, name: 'mainLanguage', contextPath },
      { min: 2, max: 2 }
    );
  }

  if (request.phone) {
    request.phone = checkPhoneRequest(request.phone);
  }

  if (request.preferredCurrencyCodename) {
    request.preferredCurrencyCodename = Validator.validateLength(
      { value: request.preferredCurrencyCodename, name: 'preferredCurrencyCodename', contextPath },
      { min: 3, max: 3 }
    );
  }
...
export const checkWritablePublicUser = (
  request: DeepPartial<ThisInterface>,
  contextPath = ''
): ThisInterface => {
  request.avatarUrl = request.avatarUrl ?? null;

  request.birthDate = Validator.validateExistence({
    value: request.birthDate,
    name: 'birthDate',
    contextPath,
  });

  request.firstName = Validator.validateExistence({
    value: request.firstName,
    name: 'firstName',
    contextPath,
  });

  request.lastName = Validator.validateExistence({
  ...
  return newWritablePublicUser({
    ...checkWritablePublicGenericObject(request),
